...
export const cacheDidDoc = async (
  ctx: CdnContext,
  did: string,
  didDoc: DidDocument,
) => {
  await ctx.env.didCache.put(did, JSON.stringify(didDoc), {
    expirationTtl: 3600,
  });
};

const blobVerificationKey = (pdsUrl: string, did: string, cid: string) =>
  `${pdsUrl}:${did}:${cid}`;
export const getCachedBlobVerification = async (
  ...
  const verified = await ctx.env.cidCache.get(
